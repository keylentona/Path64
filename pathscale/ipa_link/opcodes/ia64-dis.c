	...
	switch (odesc->class)
	  ...
	  case IA64_OPND_CLASS_ABS:
	    ...
	    if (str)
	      ...
	      (*info->fprintf_func) (info->stream, "%" SCNd64 "", (long long) value);
	    else if (odesc->flags & IA64_OPND_FLAG_DECIMAL_UNSIGNED)
	      (*info->fprintf_func) (info->stream, "%" SCNu64 "", (long long) value);
	    else
	      (*info->fprintf_func) (info->stream, "0x%" SCNx64 "", (long long) value);
	    break;
