...
static bfd_boolean gldelf_x86_64_place_orphan (asection *s);
  ...
  if (! initialized)
    {
      ...
      tmppath = concat (ld_sysroot, "/opt/pathscale/etc/ld.so.conf", NULL);
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
	      ...
	      if (gldelf_x86_64_search_needed (lib_path, &n,
						      force))
		break;
	    }
	  lib_path = (const char *) getenv ("LD_LIBRARY_PATH");
	  if (gldelf_x86_64_search_needed (lib_path, &n, force))
	    break;
	  found = 0;
